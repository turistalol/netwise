...
const features: Feature[] = [
  ...
  // Side Features (Right - Small stacked)
  {
    ...
    description: 'Supervisão proativa com alertas em tempo real e dashboards avançados.',
    icon: Activity,
  ...
  },
  {
    ...
    description: 'Sistema automatizado com versionamento e recuperação instantânea.',
    icon: Database,
  ...
  // Bottom Features (Bottom row)
  {
    ...
    description: 'Storage com velocidades superiores a 7.000 MB/s para máxima performance.',
    icon: HardDrive,
  ...
  {
    name: 'Segurança Multicamadas',
    description: 'Firewall corporativo, WAF e compliance com padrões internacionais.',
    icon: Shield,
  ...
  },
  {
    ...
    description: 'Escalabilidade automática baseada em IA para otimizar recursos.',
    icon: Cloud,
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-8 z-10">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-foreground">
            Tecnologia de<span className="text-accent">Virtual Machine</span> de ponta
          </h2>
          <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
            Infraestrutura projetada para suportar as aplicações mais exigentes
            com performance, segurança e disponibilidade garantidas.
          </p>
        </motion.div>
        
        <AsymmetricGrid features={features} />
      </div>
    </section>
  )
}

function AsymmetricGrid({ features }: { features: Feature[] }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        >
          <MainFeatureCard feature={mainFeature} />
        </motion.div>
        ...
        <div className="flex flex-col gap-6">
          {sideFeatures.map((feature, idx) => (
            <motion.div
              ...
              viewport={{ once: true, margin: "-50px" }}
            >
              <SideFeatureCard feature={feature} />
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
        {bottomFeatures.map((feature, idx) => (
          <motion.div
            ...
            viewport={{ once: true, margin: "-50px" }}
          >
            <BottomFeatureCard feature={feature} />
          </motion.div>
        ))}
      </div>
    </div>
...
function MainFeatureCard({ feature }: { feature: Feature }) {
...
function SideFeatureCard({ feature }: { feature: Feature }) {
  const Icon = feature.icon;
...
function BottomFeatureCard({ feature }: { feature: Feature }) {
  const Icon = feature.icon;
